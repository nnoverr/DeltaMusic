// DeltaMusic Local Proxy Server
// Serves both static PWA files and proxies requests to hitmotop
...
function proxyRequest(targetUrl, res) {
    ...
    const req = proto.request(options, (upstream) => {
        ...
        if (upstream.statusCode >= 300 && upstream.statusCode < 400 && upstream.headers.location) {
            proxyRequest(upstream.headers.location, res);
            return;
        }
...
server.listen(PORT, () => {
    console.log(`\n  DeltaMusic server running at:\n  http://localhost:${PORT}\n  http://127.0.0.1:${PORT}\n`);
    console.log('  Proxy endpoint: http://localhost:3000/proxy?url=<encoded_url>');
});
